class QymhScroll{
  ...
  init(){
    ...
    /** 事件 **/
    window.addEventListener('scroll',this.heartBit(()=>this.scrollerEvent()))
    ...
    $bornEl.style.paddingBottom=this.offsetBottom+'px'
   ...
   * @param {Number} bit 执行时间
   */
  heartBit(fn,bit=100){
    let outerDate=new Date().getTime()
    return ()=>{
      let innerDate=new Date().getTime()
      if(innerDate-outerDate>bit){
        outerDate=innerDate
        fn()
      }
    }
  ...
  scrollerEvent(){
    ...
    if(bool&&!this.isLoading&&scrollTop!==0){
      this.baseLoadingFn()
    }
  }
   ...
   */
  resetScroll(){
    this.finishDom.style.display='none'
    this.loadingDom.style.display='flex'
    this.isFinish=false
    this.isLoading=false
